...
export function PriorityPillSelector({ priority, onChange }: PriorityPillSelectorProps) {
    ...
    const activeOption = PRIORITY_OPTIONS.find(opt => opt.value === priority) || PRIORITY_OPTIONS[3];
    
    // Fallback styling if none
    const isNone = priority === 'none' || !priority;
    const pillColor = isNone ? 'var(--text-muted)' : activeOption.colorVar;
    const pillBg = isNone ? 'var(--bg-tertiary)' : `color-mix(in srgb, ${activeOption.colorVar} 15%, transparent)`;
    const pillBorder = isNone ? 'transparent' : `color-mix(in srgb, ${activeOption.colorVar} 30%, transparent)`;
