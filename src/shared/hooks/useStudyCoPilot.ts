...
export const useStudyCoPilot = () => {
    const { progress, mockScores, studySessions, handleAddPlannerTask, handleUpdateChapterDetail } = useUserProgress();
    ...
    const subjectWeaknessWeights = useMemo(() => {
        ...
        let totalPhy = 0, totalChem = 0, totalMath = 0;
    ...
    const recommendations = useMemo(() => {
        ...
        subjects.forEach(sub => {
            ...
            subjectChapters.forEach((chapter: Chapter) => {
                ...
                const totalItems = chapter.subtopics && chapter.subtopics.length > 0 
                ...
                if (sessions.length > 0) {
                    const latestSessionTime = new Date(Math.max(...sessions.map(s => new Date(s.endTime).getTime())));
                ...
                if (lastActiveTime) {
                    daysSinceActive = Math.max(0, Math.floor((today.getTime() - lastActiveTime.getTime()) / (1000 * 60 * 60 * 24)));
                ...
                // Completion Boost
                const completionBoost = (completionRate >= 0.70 && completionRate < 1.0) ? 0.25 : 0.0;

                // Calculate Urgency Index
                const baseUrgency = (wp * 35) + (uc * 25) + (staleness * 40);
                const urgencyIndex = Math.round(Math.min(100, baseUrgency * wTime * wMock * (1 + completionBoost)));
    ...
    }, [progress, mergedSubjectData, subjectStudyTimeShares, subjectWeaknessWeights, chapterSessionsMap]);

    // 5. Inject task into planner
    const addRevisionToPlanner = useCallback((rec: CoPilotRecommendation) => {
        const todayStr = new Date().toISOString().split('T')[0];
        const task: PlannerTask = {
            id: `copilot-revision-${Date.now()}`,
            title: `Revise: ${rec.chapterName}`,
            subtitle: rec.type === 'mock_boost' 
                ? 'Weakness Mock Nudge' 
    ...
    const markChapterRevised = useCallback((subject: Subject, chapterSerial: number, confidence?: ConfidenceLevel) => {
        const todayStr = new Date().toISOString().split('T')[0];
        ...
        const updatedHistory = [
            ...currentHistory,
            { date: todayStr, confidence: newConfidence }
        ];
