...
export const CoPilotNudgeRow: React.FC<CoPilotNudgeRowProps> = ({
    ...
    const getVariantDetails = () => {
        switch (type) {
            case 'stuck': return { icon: <AlertTriangle className="nudge-row-icon icon-stuck" size={20} />, reason: 'Stuck Alert', colorClass: 'row-variant-stuck' };
            case 'mock_boost': return { icon: <TrendingDown className="nudge-row-icon icon-mock" size={20} />, reason: 'Mock Score Boost', colorClass: 'row-variant-mock' };
            case 'neglect_balance': return { icon: <Clock className="nudge-row-icon icon-neglect" size={20} />, reason: 'Balance Neglected Subject', colorClass: 'row-variant-neglect' };
            case 'close_to_complete': return { icon: <Sparkles className="nudge-row-icon icon-complete" size={20} />, reason: 'Almost Complete!', colorClass: 'row-variant-complete' };
            case 'revision':
            default: return { icon: <Brain className="nudge-row-icon icon-revision" size={20} />, reason: 'Critical Revision Needed', colorClass: 'row-variant-revision' };
        }
    };
    ...
    return (
        <div className={`nudge-row ${variant.colorClass}`}>
            ...
            <div className="nudge-col-metrics">
                <div className={`compact-metric ${urgencyIndex > 75 ? 'metric-high' : ''}`} data-tooltip="Based on time decay, mock scores, & syllabus weight">
                    <span className="c-metric-lbl">Urgency</span>
                    <span className="c-metric-val">{urgencyIndex}%</span>
                </div>
                {metadata.retention !== undefined && (
                    <div className={`compact-metric ${metadata.retention < 20 ? 'metric-low-retention' : ''}`} data-tooltip="Estimated memory retention based on time elapsed">
                ...
                {metadata.daysSinceActive !== undefined && (
                    <div className="compact-metric muted" data-tooltip="Days since chapter was last actively revised">
            ...
            <div className="nudge-col-actions">
                {showConfidenceSelector ? (
                    <div className="inline-confidence-selector">
                        ...
                        <button className="inline-cancel-btn" onClick={() => setShowConfidenceSelector(false)} title="Cancel">
                            <X size={14} />
                        </button>
                    </div>
                ) : (
                    <div className="inline-action-group">
                        ...
                        <button 
                            className="btn-compact ghost icon-only" 
