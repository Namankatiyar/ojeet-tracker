...
export const DailyAnalytics: React.FC = () => {
  ...
  const { momentumTrend, avg3Day } = useMemo(() => {
    ...
    if (totalDurationSec === 0 && prevTotalSec === 0) trend = 'stable';
      ...
      Math.abs(totalDurationSec - prevTotalSec) <= prevTotalSec * threshold &&
      prevTotalSec > 0
    )
      trend = 'stable';
  ...
  const insights = useMemo(() => {
    ...
    if (hasLateNight) {
      items.push({
        text: (
          <p>You studied past 10 PM. Get 7–8 hours of sleep for optimal memory consolidation.</p>
        ),
        type: 'warn',
      });
      ...
      items.push({
        text: (
          <p>
            Solid output today! Your peak window aligns with JEE exam timings — keep reinforcing
  ...
  const renderDelta = () => {
    ...
    if (isToday) {
      const label =
        diffSec >= 0
          ? `+${formatSmartDuration(diffSec)}`
          : `-${formatSmartDuration(Math.abs(diffSec))}`;
      const badgeClass = diffSec >= 0 ? 'dh-badge success' : 'dh-badge danger';
      return (
        <span className={badgeClass} title="Day in progress">
          {label} (so far)
        </span>
      );
    }
  ...
  const weeklyData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const totals: Record<Subject | 'custom', number> = {
        physics: 0,
        chemistry: 0,
        maths: 0,
        biology: 0,
  ...
  const weeklyMax = useMemo(() => Math.max(...weeklyData.map((d) => d.total), 1), [weeklyData]);
                ...
                <div className="dh-metrics-grid">
                  ...
                  <div className="dh-metric-block">
                    ...
                    <div className="dh-metric-meta">
                      {renderDelta()}
                      {(prevTotalSec !== 0 || totalDurationSec !== 0) && (
                        <span className="dh-metric-vs">vs {prevDayName}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="dh-metric-block dh-metric-block--secondary">
                    ...
                    <div className="dh-metric-meta">
                      {renderQuestionDelta()}
                      {(prevQuestions !== 0 || todayQuestions !== 0) && (
                        <span className="dh-metric-vs">vs {prevDayName}</span>
                      )}
                    </div>
                  </div>
                ...
                <div className="dh-hero-timeline-header">
                  ...
                  {peakHour >= 0 && (
                    ...
                    >
                      <Zap size={10} />
                      <span>
                        Peak: <strong>{formatHourLabel(peakHour)}</strong>
                      </span>
                    </div>
                  )}
                </div>
                <div className="dh-histogram-container compact">
                  <div className="dh-histogram-grid">
                    {hourlyBuckets.map((slot, hr) => {
                      const heightPct = histogramMax > 0 ? (slot.totalMin / histogramMax) * 100 : 0;
                      const subArr = Array.from(slot.subjects);
                      const colorClass =
                        slot.totalMin > 0 ? (subArr.length === 1 ? subArr[0] : 'mixed') : 'empty';
                      ...
                      return (
                        <div key={hr} className="dh-histogram-col" data-tooltip={tooltip}>
                  ...
                  <div className="dh-timeline-axis">
                    ...
                    {(
                      ...
                      <div key={i} className="dh-timeline-axis-pin" style={{ left: `${pct}%` }}>
            ...
            <div className="dh-bento-stats">
              <div className="dh-bento-unified-stats glass-panel">
                ...
                <div className="dh-stat-section">
                  <div className="dh-card-header dh-card-header-flush">
                    {momentumTrend === 'improving' ? (
                      <TrendingUp size={14} className="dh-card-header-icon" />
                    ) : momentumTrend === 'declining' ? (
                      <TrendingDown size={14} className="dh-card-header-icon" />
                    ) : (
                      <Activity size={14} className="dh-card-header-icon" />
                    )}
                    <h3>Momentum</h3>
                  ...
                  <span className="dh-stat-sub">3-day avg: {formatSmartDuration(avg3Day)}</span>
                </div>
              </div>
            </div>
              ...
              <div className="dh-card-header">
                ...
                {subjectTotals.total > 0 && (
                  <div className="dh-total-studied-badge">
                    <Clock size={10} />
                    <span>
                      Total: <strong>{formatSmartDuration(subjectTotals.total)}</strong>
                    </span>
                  </div>
                )}
              </div>
                  ...
                  <div className="dh-subject-card" key={subject}>
                    <div className="dh-subject-card-header">
                      ...
                      <div className="dh-subject-card-stats">
                        <span className="dh-subject-card-time">
                          {formatSmartDuration(duration)}
                        </span>
                        ...
                        {chapters.map((ch) => (
                          <div className="dh-chapter-card" key={ch.name}>
                            <div className="dh-chapter-card-top">
                              ...
                              <span className="dh-chapter-card-time">
                                {formatSmartDuration(ch.duration)}
                              </span>
                            </div>
                            {ch.materials.size > 0 && (
                              <div
                                className="dh-chapter-card-tags"
                                title={Array.from(ch.materials).join(', ')}
                              >
