...
export function ImportSyncPage({ onAddSession }: ImportSyncPageProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const importHistory: string[] = JSON.parse(localStorage.getItem(IMPORT_HISTORY_KEY) || '[]');
      ...
      localStorage.setItem(IMPORT_HISTORY_KEY, JSON.stringify([...importHistory, data.timestamp]));
      ...
      setStatus('error');
      setErrorMessage(err instanceof Error ? err.message : 'Failed to parse sync data.');
    }
      ...
      <div className="import-sync-card glass-panel">
        ...
        {status === 'success' && (
          <div className="import-sync-content">
            ...
            <h2>Sync Complete!</h2>
            <p>
              Imported <strong>{importedCount} sessions</strong> ({importedMinutes} min) from
              OJEET-STUDY into your study history.
            </p>
