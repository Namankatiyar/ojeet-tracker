...
export function InviteHandler() {
    ...
    useEffect(() => {
        ...
        const fetchSenderProfile = async () => {
            ...
            try {
                const { data, error } = await client.rpc('get_profile_by_invite_code', { friend_code: inviteCode });
                    ...
                    {connectionStatus === 'success' ? (
                        <div className="invite-status-view success">
                            ...
                            <p className="invite-status-desc">
                                You and your friend are now connected as study peers. You can now view each other's streaks, daily agenda, and progress on the community page.
                            </p>
                            <button className="primary-btn" onClick={handleGoToCommunity} style={{ margin: 'var(--space-4) auto 0' }}>
                        ...
                        <div className="invite-status-view error">
                            ...
                            <p className="invite-error-desc" style={{ color: 'var(--priority-high)' }}>{errorMessage}</p>
                            <div className="invite-modal-actions" style={{ width: '100%', gap: 'var(--space-2)', marginTop: 'var(--space-4)', display: 'flex' }}>
        ...
        <div className="invite-handler-page">
            <div className="invite-handler-card glass-panel">
                <div className="invite-landing-header">
                    {senderProfile ? (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                            ...
                            />
                            <h2 style={{ marginTop: 'var(--space-3)' }}>Connect with {senderProfile.display_name}</h2>
                        </div>
                    ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <div className="invite-landing-body" style={{ marginTop: 'var(--space-4)' }}>
                    <p className="invite-landing-desc" style={{ color: 'var(--text-secondary)', textAlign: 'center', marginBottom: 'var(--space-4)' }}>
                        Connect with friends to share your JEE syllabus progress, time study sessions, check daily agendas, and keep each other accountable.
                    </p>
