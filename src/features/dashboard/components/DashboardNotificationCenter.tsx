...
export function DashboardNotificationCenter({ items, onPanelOpen }: DashboardNotificationCenterProps) {
            ...
            {isOpen && (
                <section className="dashboard-notifications-panel glass-panel" aria-label="Notifications">
                    ...
                    <div className="dashboard-notifications-list">
                        {items.map((item) => (
                            <article key={item.id} className={`dashboard-notification-item ${item.unread ? 'is-unread' : ''}`}>
