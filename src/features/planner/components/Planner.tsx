...
export function Planner({
  ...
  const totalStudyTime = useMemo(() => {
    ...
    const relevantSessions = sessions.filter(
      (s) => s.startTime.slice(0, 10) >= period.start && s.startTime.slice(0, 10) <= period.end
    );
  ...
  const handlePrev =
    viewMode === 'monthly' ? dateNavigator.handlePrevMonth : dateNavigator.handlePrevWeek;
  const handleNext =
    viewMode === 'monthly' ? dateNavigator.handleNextMonth : dateNavigator.handleNextWeek;
  const currentLabel =
