...
function renderMarkdown(text: string): string {
    ...
    const parts = html.split(/(<pre[\s\S]*?<\/pre>|<ul[\s\S]*?<\/ul>|<ol[\s\S]*?<\/ol>|<h[1-3]>.*?<\/h[1-3]>)/g);
    const processedParts = parts.map(part => {
        if (part.startsWith('<pre') || part.startsWith('<ul') || part.startsWith('<ol') || part.startsWith('<h')) {
            return part;
        }
        let p = part.trim();
        if (!p) return '';
        ...
        <div className={`chat-bubble-row ${isUser ? 'chat-bubble-row--user' : ''}`}>
            ...
            <div className="chat-bubble-container">
                <div className={`chat-bubble ${isUser ? 'chat-bubble--user' : 'chat-bubble--model'}`}>
                    {isStreaming ? (
                        <div className="chat-streaming-container" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                            {content && (
                                <span 
            ...
            {hasKey ? (
                ...
                <div className="chat-apikey-form">
                    <input
                        ...
                        autoComplete="off"
                    />
                    <button id="chat-apikey-save" className="chat-apikey-save-btn" onClick={handleSave} aria-label="Save API Key">
                        Save
                    </button>
                </div>
            )}
...
export function ChatDrawer() {
    ...
    const { 
        ...
        messages, isLoading, sendMessage, confirmPendingAction, cancelPendingAction, clearActiveSession 
    } = useAgentChat();
                ...
                {activeView === 'history' ? (
                    ...
                    <div className="chat-sessions-list">
                        <div className="chat-sessions-header">
                            ...
                            <button className="chat-new-session-btn" onClick={() => { startNewSession(); setActiveView('chat'); }}>
                    ...
                    <>
                                ...
                                {!loadApiKey() ? (
                                    <div className="chat-welcome-card chat-welcome-card--setup glass-panel">
                                        ...
                                        <p className="chat-welcome-desc">
                                            Blue runs entirely in your browser using your own API key. Setting it up is free and takes less than a minute.
                                        </p>
                                        <div className="chat-setup-instructions">
                                            <ol>
                                                <li>Go to <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer">Google AI Studio</a> and click <strong>Get API key</strong>.</li>
                                                <li>Copy your generated key (starts with <code>AIza</code>).</li>
                                                <li>Paste the key into the setup field below and click <strong>Save</strong>.</li>
                                            </ol>
                                        </div>
                                    ...
                                    <>
                                        <div className="chat-welcome-card glass-panel">
                                            ...
                                            <p className="chat-welcome-desc">
                                                Your AI study companion. Ask me to track syllabus progress, schedule tasks, log mock scores, or suggest revision plans.
                                            </p>
                                        </div>
                        ...
                        {loadApiKey() && (
                            <div className="chat-input-area">
                                <div className={`chat-input-wrapper ${isLoading ? 'chat-input-wrapper--disabled' : ''}`}>
                                    <textarea
