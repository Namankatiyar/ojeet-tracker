...
import { Subject, Priority, PlannerTask, StudySession, ConfidenceLevel, Chapter } from '../../../shared/types';
...
function fuzzyFindSubtopic(subtopics: string[], query: string): string | null {
    ...
    // Includes match
    const includes = subtopics.find(s => s.toLowerCase().includes(q) || q.includes(s.toLowerCase()));
...
function fuzzyFindChapter(chapters: Chapter[], query: string): Chapter | null {
    ...
    if (exact) return exact;
    const includes = chapters.find(c => c.name.toLowerCase().includes(q) || q.includes(c.name.toLowerCase()));
    if (includes) return includes;
    return null;
}
...
export function useAgentTools() {
    const {
        plannerTasks, mockScores, studySessions, examDates, progress,
        handleToggleMaterial, handleSetPriority,
        handleToggleSubtopicMaterial, handleUpdateSubtopicAttempted, handleSetSubtopicLastRevised,
        handleAddPlannerTask, handleTogglePlannerTask, handleDeletePlannerTask,
        handleAddStudySession, handleDeleteStudySession,
        handleAddMockScore, handleDeleteMockScore,
        handleAddExam, handleDeleteExam, handleSetPrimaryExam,
    } = useUserProgress();
    ...
    // ── PROGRESS TOOLS ────────────────────────────────────────────────────────
    const toggleChapterMaterial = useCallback((subject: Subject, chapterName: string, material: string): string => {
    ...
    const toggleSubtopicMaterial = useCallback((subject: Subject, chapterName: string, subtopicName: string, material: string): string => {
        const chapters = mergedSubjectData[subject]?.chapters ?? [];
        const chapter = fuzzyFindChapter(chapters, chapterName);
        if (!chapter) return err(`Chapter "${chapterName}" not found in ${subject}.`);
        const subtopics = chapter.subtopics ?? [];
        const subtopic = fuzzyFindSubtopic(subtopics, subtopicName);
        if (!subtopic) return err(`Subtopic "${subtopicName}" not found in chapter "${chapter.name}".`);
        handleToggleSubtopicMaterial(subject, chapter.serial, subtopic, material);
        return ok(`Toggled "${material}" for subtopic "${subtopic}" in ${subject} "${chapter.name}".`);
    }, [mergedSubjectData, handleToggleSubtopicMaterial]);

    const updateSubtopicAttempted = useCallback((subject: Subject, chapterName: string, subtopicName: string, material: string, count: number): string => {
        const chapters = mergedSubjectData[subject]?.chapters ?? [];
        const chapter = fuzzyFindChapter(chapters, chapterName);
    ...
    const setSubtopicLastRevised = useCallback((subject: Subject, chapterName: string, subtopicName: string, date: string | undefined): string => {
        const chapters = mergedSubjectData[subject]?.chapters ?? [];
        const chapter = fuzzyFindChapter(chapters, chapterName);
        if (!chapter) return err(`Chapter "${chapterName}" not found.`);
        const subtopics = chapter.subtopics ?? [];
    ...
    const setChapterPriority = useCallback((subject: Subject, chapterName: string, priority: Priority): string => {
        const chapters = mergedSubjectData[subject]?.chapters ?? [];
        const chapter = fuzzyFindChapter(chapters, chapterName);
        if (!chapter) return err(`Chapter "${chapterName}" not found.`);
    ...
    const markChapterRevisedTool = useCallback((subject: Subject, chapterName: string, confidence?: ConfidenceLevel): string => {
        const chapters = mergedSubjectData[subject]?.chapters ?? [];
        const chapter = fuzzyFindChapter(chapters, chapterName);
        if (!chapter) return err(`Chapter "${chapterName}" not found.`);
    ...
    const deletePlannerTask = useCallback((taskId: string): string => {
        const task = plannerTasks.find(t => t.id === taskId);
        if (!task) return err(`Task "${taskId}" not found.`);
    ...
    const scheduleRevision = useCallback((subject: Subject, chapterName: string): string => {
        const chapters = mergedSubjectData[subject]?.chapters ?? [];
        const chapter = fuzzyFindChapter(chapters, chapterName);
        if (!chapter) return err(`Chapter "${chapterName}" not found.`);
    ...
    const listPlannerTasks = useCallback((dateFilter?: string): string => {
        const tasks = dateFilter
            ? plannerTasks.filter(t => t.date === dateFilter)
            : plannerTasks.filter(t => !t.completed).slice(0, 20);
        return ok(`Found ${tasks.length} tasks.`, tasks.map(t => ({
            id: t.id, title: t.title, date: t.date, time: t.time,
            completed: t.completed, subject: t.subject
        })));
    }, [plannerTasks]);

    // ── STUDY SESSION TOOLS ───────────────────────────────────────────────────
    const logStudySession = useCallback((
        title: string, durationMinutes: number, subject?: Subject,
        chapterName?: string, material?: string, date?: string
    ): string => {
        ...
        if (subject && chapterName) {
            const chapters = mergedSubjectData[subject]?.chapters ?? [];
            const ch = fuzzyFindChapter(chapters, chapterName);
            if (ch) { chapterSerial = ch.serial; chapterNameResolved = ch.name; }
        }
    ...
    const deleteStudySession = useCallback((sessionId: string): string => {
    ...
    const listStudySessions = useCallback((dateFilter?: string): string => {
        const sessions = dateFilter
            ? studySessions.filter(s => (s.localDate ?? s.startTime.split('T')[0]) === dateFilter)
            : [...studySessions].sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime()).slice(0, 10);
        return ok(`Found ${sessions.length} sessions.`, sessions.map(s => ({
            id: s.id, title: s.title, subject: s.subject,
            duration: s.duration, localDate: s.localDate
        })));
    }, [studySessions]);
    ...
    const addMockScore = useCallback((
        ...
        physicsMarks: number, chemistryMarks: number, mathsMarks: number,
        maxMarks?: number, examType?: string
    ): string => {
        const totalMarks = physicsMarks + chemistryMarks + mathsMarks;
        handleAddMockScore({
            name, date, examType,
            physicsMarks, chemistryMarks, mathsMarks,
            totalMarks, maxMarks: maxMarks ?? 300,
        });
        return ok(`Logged mock "${name}" on ${date}: P${physicsMarks} C${chemistryMarks} M${mathsMarks} = ${totalMarks}.`);
    }, [handleAddMockScore]);

    const deleteMockScore = useCallback((scoreId: string): string => {
    ...
    const listMockScores = useCallback((): string => {
        const scores = [...mockScores]
            ...
            .slice(0, 10);
        return ok(`Found ${scores.length} mock scores.`, scores.map(s => ({
            id: s.id, name: s.name, date: s.date,
            total: s.totalMarks, max: s.maxMarks ?? 300
        })));
    }, [mockScores]);

    // ── EXAM DATE TOOLS ───────────────────────────────────────────────────────
    const addExamDate = useCallback((name: string, date: string, isPrimary: boolean = false): string => {
        handleAddExam({ name, date, isPrimary });
        return ok(`Added exam "${name}" on ${date}.`);
    }, [handleAddExam]);

    const deleteExamDate = useCallback((examId: string): string => {
    ...
    const setPrimaryExam = useCallback((examId: string): string => {
        const exam = examDates.find(e => e.id === examId);
        if (!exam) return err(`Exam "${examId}" not found.`);
    ...
    // ── READ/QUERY TOOLS ───────────────────────────────────────────────────────
    const getChapterProgress = useCallback((subject: Subject, chapterName: string): string => {
        const chapters = mergedSubjectData[subject]?.chapters ?? [];
        const chapter = fuzzyFindChapter(chapters, chapterName);
        if (!chapter) return err(`Chapter "${chapterName}" not found.`);
        const chapProgress = progress[subject]?.[chapter.serial];
        return ok(`Progress for ${subject} "${chapter.name}".`, {
            chapter: { serial: chapter.serial, name: chapter.name, materials: chapter.materials, subtopics: chapter.subtopics },
            progress: chapProgress,
        });
    }, [mergedSubjectData, progress]);

    const getSubjectChapters = useCallback((subject: Subject): string => {
        const data = mergedSubjectData[subject];
        if (!data) return err(`Subject "${subject}" data not loaded.`);
        return ok(`Chapters for ${subject}.`, data.chapters.map(c => ({
            serial: c.serial, name: c.name, materials: c.materials,
    ...
    const executors: Record<string, (args: Record<string, any>) => string> = useMemo(() => ({
        ...
        toggle_chapter_material: (args: Record<string, unknown>) =>
            toggleChapterMaterial(args.subject as Subject, args.chapter_name as string, args.material as string),
        toggle_subtopic_material: (args: Record<string, unknown>) =>
            toggleSubtopicMaterial(args.subject as Subject, args.chapter_name as string, args.subtopic_name as string, args.material as string),
        update_subtopic_attempted: (args: Record<string, unknown>) =>
            updateSubtopicAttempted(args.subject as Subject, args.chapter_name as string, args.subtopic_name as string, args.material as string, args.count as number),
        set_subtopic_last_revised: (args: Record<string, unknown>) =>
            setSubtopicLastRevised(args.subject as Subject, args.chapter_name as string, args.subtopic_name as string, args.date as string | undefined),
        set_chapter_priority: (args: Record<string, unknown>) =>
            setChapterPriority(args.subject as Subject, args.chapter_name as string, args.priority as Priority),
        ...
        log_study_session: (args: Record<string, unknown>) =>
            logStudySession(args.title as string, args.duration_minutes as number, args.subject as Subject | undefined, args.chapter_name as string | undefined, args.material as string | undefined, args.date as string | undefined),
        ...
        add_mock_score: (args: Record<string, unknown>) =>
            addMockScore(args.name as string, args.date as string, args.physics_marks as number, args.chemistry_marks as number, args.maths_marks as number, args.max_marks as number | undefined, args.exam_type as string | undefined),
        delete_mock_score: (args: Record<string, unknown>) =>
            deleteMockScore(args.score_id as string),
        list_mock_scores: (_args: Record<string, unknown>) =>
            listMockScores(),
        // Exams
        add_exam_date: (args: Record<string, unknown>) =>
            addExamDate(args.name as string, args.date as string, args.is_primary as boolean | undefined),
        delete_exam_date: (args: Record<string, unknown>) =>
            deleteExamDate(args.exam_id as string),
        list_exam_dates: (_args: Record<string, unknown>) =>
            listExamDates(),
        set_primary_exam: (args: Record<string, unknown>) =>
            setPrimaryExam(args.exam_id as string),
        // Read
        ...
        toggleChapterMaterial, toggleSubtopicMaterial, updateSubtopicAttempted,
        setSubtopicLastRevised, setChapterPriority, markChapterRevisedTool,
        addPlannerTask, togglePlannerTask, deletePlannerTask, listPlannerTasks, scheduleRevision,
        logStudySession, deleteStudySession, listStudySessions,
        addMockScore, deleteMockScore, listMockScores,
        addExamDate, deleteExamDate, listExamDates, setPrimaryExam,
        getChapterProgress, getSubjectChapters,
    ]);
    ...
    const toolDeclarations: Tool = useMemo((): Tool => {
        const declarations: FunctionDeclaration[] = [
            // ── READ TOOLS ──
            {
                ...
                description: 'List all chapters for a subject with their serial numbers, materials, and subtopic counts.',
                parameters: {
                    type: Type.OBJECT,
                    properties: {
                        subject: { type: Type.STRING, description: 'One of: physics, chemistry, maths', enum: ['physics', 'chemistry', 'maths'] },
                    },
                    required: ['subject'],
                },
            },
            {
                ...
                description: 'Get detailed progress for a specific chapter, including completed materials and subtopic states.',
                parameters: {
                    type: Type.OBJECT,
                    properties: {
            ...
            // ── PROGRESS TOOLS ──
            {
                ...
                description: 'Toggle a chapter-level material (e.g. NCERT, PYQs, Modules) as completed/incomplete. This marks ALL subtopics at once.',
                parameters: {
                    type: Type.OBJECT,
                    properties: {
                        subject: { type: Type.STRING, enum: ['physics', 'chemistry', 'maths'] },
