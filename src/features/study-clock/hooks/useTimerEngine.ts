  ...
  if (state.mode === 'pomodoro') {
    ...
    if (state.phase === 'work') {
      ...
      const phase: TimerPhase =
        cycleCount % pom.cyclesBeforeLongBreak === 0 ? 'longBreak' : 'shortBreak';
      return {
        ...state,
        phase,
        cycleCount,
...
export function reconcileTimerState(state: PersistedTimerState, now: number): ReconcileResult {
  ...
  while (
    ...
    elapsedMs >= nextState.durationMs
  ) {
...
export function useTimerEngine(options: UseTimerEngineOptions = {}): UseTimerEngineReturn {
  ...
  const reconcileNow = useCallback(
    (sourceState?: PersistedTimerState, nowOverride?: number) => {
