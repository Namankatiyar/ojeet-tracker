...
export function SessionStatistics({ sessions, subjectData }: SessionStatisticsProps) {
  ...
  const getSessionsForPeriod = useCallback(
    (period: 'overall' | 'daily' | 'weekly' | 'monthly') => {
      const now = new Date();
  ...
  const statsAvailableMaterials: string[] =
    statsSubject !== 'all'
      ? Array.from(
          new Set(
            sessions
              .filter(
                (s) =>
                  s.subject === statsSubject &&
                  (statsChapter === 'all' || s.chapterSerial === statsChapter)
              )
              .map((s) => s.material)
          )
  ...
  const getTopChaptersForSubject = (subject: Subject) => {
    ...
    activeSessions
      ...
      .forEach((s) => {
        const current = chapterTimes.get(s.chapterSerial!) || {
          name: s.chapterName || `Chapter ${s.chapterSerial}`,
          time: 0,
        };
          ...
          <div className="stats-filtered-label">
            {statsSubject === 'all' ? (
              ...
              <>
                ...
                {statsChapter !== 'all' &&
                  ` > ${subjectData[statsSubject]?.chapters.find((c) => c.serial === statsChapter)?.name || ''}`}
                ...
                return (
                  <div key={item.key} className="distribution-subject-section">
                    <div
                      ...
                      onClick={() =>
                        item.key !== 'custom' && toggleChapterGraph(item.key as Subject)
                      }
                    ...
                    >
                      <div className="distribution-bar-header">
                        ...
                        <span className="distribution-bar-value">
                          ...
                          {activeTotalTime > 0
                            ? Math.round((item.time / activeTotalTime) * 100)
                            : 0}
                          %)
                        </span>
                      </div>
                      <div className="distribution-bar-track">
                        <div
                          ...
                          style={{
                            width: `${activeTotalTime > 0 ? (item.time / activeTotalTime) * 100 : 0}%`,
                          }}
                        />
                      </div>
                    </div>
                            ...
                            topChapters.map((chapter) => (
                              <div key={chapter.serial} className="chapter-bar-item">
                                <div className="chapter-bar-header">
                                  ...
                                  <span className="chapter-bar-time">
