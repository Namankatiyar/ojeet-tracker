...
describe('buildBucketEntry', () => {
    ...
    it('fills missing fields from a partial input', () => {
        expect(buildBucketEntry({ physics: 42 })).toEqual({ overall: 0, physics: 42, chemistry: 0, maths: 0 });
    });
...
describe('addToBucket', () => {
    ...
    it('accumulates seconds into an existing bucket', () => {
        const map: AggregateBucketMap = { '2026-03-07': { overall: 50, physics: 50, chemistry: 0, maths: 0 } };
        addToBucket(map, '2026-03-07', 'physics', 100);
...
describe('computeLocalStudyAggregate', () => {
    ...
    it('populates daily/weekly/monthly buckets', () => {
        const agg = computeLocalStudyAggregate([
            makeSession({ id: '1', subject: 'physics', duration: 1000, startTime: '2026-03-07T10:00:00.000Z' }),
        ]);
    ...
    it('handles high volume: 1000 sessions', () => {
        ...
        expect(agg.total_seconds_physics + agg.total_seconds_chemistry + agg.total_seconds_maths).toBe(1000 * 1800);
    });
...
describe('mergeRemoteVideoLogsIntoSessions', () => {
    ...
    it('does not duplicate when local already has the same video', () => {
        ...
        const result = mergeRemoteVideoLogsIntoSessions(existing, [makeVideoEntry({ watched_seconds: 3600 })]);
        expect(result.changed).toBe(false);
    });
    ...
    it('ignores video entries with invalid watched_date', () => {
        const result = mergeRemoteVideoLogsIntoSessions([], [makeVideoEntry({ watched_date: 'not-a-date' })]);
        expect(result.changed).toBe(false);
    });
    ...
    it('sets type to "chapter" when subject is valid, "custom" otherwise', () => {
        const withSubject = mergeRemoteVideoLogsIntoSessions([], [makeVideoEntry({ subject: 'physics' })]);
    ...
    it('accumulates local video time across multiple sessions for the same video', () => {
        ...
        const result = mergeRemoteVideoLogsIntoSessions(existing, [makeVideoEntry({ video_id: 'v1', watched_seconds: 300 })]);
        expect(result.changed).toBe(false);
    });
});
...
describe('computeSessionDelta', () => {
    ...
    it('handles high volume: 500 sessions added', () => {
        const prev: StudySession[] = [];
        const curr: StudySession[] = Array.from({ length: 500 }, (_, i) => makeSession({ id: `s-${i}` }));
        const delta = computeSessionDelta(prev, curr, cid);
    ...
    it('handles high volume: 500 sessions deleted', () => {
        const prev: StudySession[] = Array.from({ length: 500 }, (_, i) => makeSession({ id: `s-${i}` }));
        const curr: StudySession[] = [];
        const delta = computeSessionDelta(prev, curr, cid);
        expect(delta).toHaveLength(500);
...
describe('applyDeltaLogs', () => {
    ...
    function makeLog(overrides: Partial<StudySessionLogEntry> & { session_id: string; action: 'INSERT' | 'DELETE' }): StudySessionLogEntry {
        return {
            id: 'log-1',
            user_id: 'user-1',
    ...
    it('does not mutate the original sessions array', () => {
        const existing = [makeSession({ id: 's1' })];
        const copy = [...existing];
        applyDeltaLogs(existing, [makeLog({ session_id: 'new', action: 'INSERT', payload: makeSession({ id: 'new' }) })], ownClientId);
